...
let a: number = 1; // Coeficiente a
let b: number = -3; // Coeficiente b
let c: number = 2; // Coeficiente c
if (a === 0) {
  console.log("A equação não é do segundo grau.");
} else {
  let delta: number = b * b - 4 * a * c; // Cálculo do delta
  if (delta < 0) {
    console.log("A equação não possui raízes reais.");
  } else if (delta === 0) {
    let raiz: number = -b / (2 * a); // Cálculo da raiz única
    console.log(`A equação possui uma raiz real: ${raiz.toFixed(2)}`);
  } else {
    let raiz1: number = (-b + Math.sqrt(delta)) / (2 * a); // Cálculo da primeira raiz
    let raiz2: number = (-b - Math.sqrt(delta)) / (2 * a); // Cálculo da segunda raiz
    console.log(
      `A equação possui duas raízes reais: ${raiz1.toFixed(
        2
      )} e ${raiz2.toFixed(2)}`
    );
  }

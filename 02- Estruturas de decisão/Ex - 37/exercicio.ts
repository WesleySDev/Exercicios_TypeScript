...
let numero: number = 326; // Número a ser analisado
if (numero <= 0 || numero >= 1000) {
  console.log("Número inválido. Deve ser maior que 0 e menor que 1000.");
} else {
  let centenas: number = Math.floor(numero / 100);
  let resto: number = numero % 100; // Resto após remover as centenas
  let dezenas: number = Math.floor(resto / 10);
  let unidades: number = resto % 10;
  let partes: string[] = []; // Array para armazenar as partes do número

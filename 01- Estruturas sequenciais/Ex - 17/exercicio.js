...
*/
let area = 54; // Área a ser pintada em metros quadrados
let coberturaPorLitro = 6; // Cobertura de tinta em metros quadrados por litro
let litrosNecessarios = area / coberturaPorLitro;
let litrosPorLata = 18;
let precoPorLata = 80;
let litrosPorGalao = 3.6;
let precoPorGalao = 25;
let folga = 1.1; // 10% de folga
...
let precoTotalLatas = latasNecessarias * precoPorLata; // Preço total das latas
let galoesNecessarios = Math.ceil(litrosNecessarios / litrosPorGalao); // Galões necessários, arredondados para cima
let precoTotalGaloes = galoesNecessarios * precoPorGalao; // Preço total dos galões
let latasParaMisturar = Math.floor(litrosNecessarios / litrosPorLata); // Latas que podem ser compradas
let litrosRestantes = litrosNecessarios - latasParaMisturar * litrosPorLata; // Litros restantes após comprar latas
let galoesParaMisturar = Math.ceil(litrosRestantes / litrosPorGalao); // Galões necessários para os litros restantes
let precoTotalMistura = latasParaMisturar * precoPorLata + galoesParaMisturar * precoPorGalao; // Preço total da mistura
...
console.log(`Preço total (misturando latas e galões): R$ ${precoTotalMistura.toFixed(2)}`);
